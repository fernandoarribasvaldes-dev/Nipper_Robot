...
class PID:
    def __init__(self, kp, ki, kd, i_limit=None):
        ...
        self.ephi_pub = self.create_publisher(Float64MultiArray, "/turret_ephi", 10)
...
class MultiTurretKinematics(Node):
    ...
    def control_step(self):
        ...
        for i, (vx_i, vy_i) in enumerate(v_turrets):
            ...
            self.e_phi_history[i].append(e_phi)
            # --------------------------
    ...
    def destroy_node(self):
        with open("ephi_log.pkl", "wb") as f:
            pickle.dump((self.time_history, self.e_phi_history), f)
        super().destroy_node()
